/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2016年10月10日 星期一 21时22分42秒
 ************************************************************************/
...
//描述一个子进程的类，m_pid是目标子进程的pid,m_pipefd是父子进程通信用的管道
class process
{
public:
    process():m_pid(-1){}
...
//进程池类，使它定义为模板类是为了代码复用，其模板参数是处理逻辑任务的类
template<typename T>
class processpool
{
private:
...
//从epollfd标识的epoll内核事件表中删除fd上的注册事件
...
//进程池构造函数，参数listenfd是监听socket,它必须在创建进程池之前被创建，否则子进程
//无法直接引用它，参数process_number指定进程池中子进程的数量
template<typename T>
...
//父进程中m_idx值为－１，子进程中m_idx大于等于０，我们据此判断接下来要
//运行的是父进程代码还是子进程代码
template<typename T>
...
void processpool<T>::run_child()
{
    ...
    while(!m_stop)
    {
        ...
        for(int i = 0; i < number; ++i)
        {
            ...
            {
                int client = 0;
                //从父子进程之间的管道读取数据，并将结果保存在变量client中，如果
                //读取成功，则表示有新客户连接到来
                ...
                else
                {
                    ...
                    //模板类T必须实现init方法，以此初始化一个客户连接，我们直接使用connfd
    ...
    while(!m_stop)
    {
        ...
        for(int i = 0; i < number; ++i)
        {
            ...
            if(sockfd == m_listenfd)
            {
                ...
                printf("senc request to child %d\n", i);
            }
            //下面处理父进程接收到的信号
            else if(sockfd == sig_pipefd[0] && (events[i].events & EPOLLIN))
            {
                        ...
                        switch(signals[i])
                        {
                            case SIGCHLD:
                            {
                                ...
                                while((pid = waitpid(-1, &stat, WNOHANG)) > 0)
                                {
                                    ...
                                    {
                                        //如果进程池中的第i个子进程退出了，则主进程关闭相应的通信
                                        //管道，并设置相应的m_pid为-1,以标记该子进程已经退出
                            ...
                            case SIGINT:
                            {
                                //如果父进程接收到终止信号，那么就杀死所有子进程，并等待它们全部
                                //结束，当然，通知子进程接收有更好的方式是向父子进程直接的通信管道
                                //发送特殊数据，读者不妨自己实现之
