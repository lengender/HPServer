/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2016年09月26日 星期一 16时12分39秒
 ************************************************************************/
...
public:
        ...
        (std::exception):cur_size(size),capacity(capacity)
    {
        ...
        array = new heap_timer*[capacity];  //创建对数组
        if(!array)
            throw std::exception();
        ...
        if(size != 0)
        {
            ...
            {
                //对数组中的第[(cur_size - 1) / 2] ~ 0 个元素执行下列操作，
                //调整为最小堆形式
                percolate_down(i);
            }
        }
...
public:
    ...
    void add_timer(heap_timer *timer) throw (std::exception)
    {
        ...
        if(cur_size >= capacity)   //如果当前堆容量不够，将其扩大１倍
            resize(); 

        //新插入一个元素，当前对大小加１，hole时新建空穴的位置
        int hole = cur_size++;
        int parent = 0;

        //对从空穴到根节点的路径上的所有节点执行下列操作
    ...
    void del_timer(heap_timer *timer)
    {
        ...
        //仅仅将目标函数定时器的回调函数设置为空，即所谓的延迟销毁，这将
        //节省真正删除该定时器造成的开销，但这样容易使堆数组膨胀
        timer->cb_func = NULL;
    }
