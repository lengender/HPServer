/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2016年10月11日 星期二 14时56分39秒
 ************************************************************************/
...
//所有用户可读，且不是目录，则使用mmap将其映射到内存地址m_file_address处，并告诉调用
//者获取文件成功
...
bool http_conn::write()
{
    ...
    while(1)
    {
        ...
        if(temp <= -1)
        {
            //如果TCP写缓冲区没有空间，则等待下一轮EPOLLOUT事件，虽然在此期间，服务
        ...
        {
            //发送http响应成功，根据http请求中connection字段决定是否立即关闭连接
            unmap();
            if(m_linger)
            {
                init();
