/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2016年09月26日 星期一 13时46分43秒
 ************************************************************************/
...
public:
    ...
    tw_timer *add_timer(int timeout)
    {
        ...
        int ticks = 0;
        //以下根据待插入定时器的超时值计算它将在时间轮转动多少滴答后被触发，
        //并将该滴答数存储于变量ticks中，如果待插入定时器的超时值小于时间轮
        //的槽间隔SI,则将ticks向上折合为１，否则就将ticks向下折合为timeout/SI
        if(timeout < SI)
            ticks = 1;
        else
            ticks = timeout / SI;
        ...
        //如果第ts个槽中尚无定时器，则把新创建的定时器插入其中，并将该定时器
        //设置为该槽的头节点
        if(!slots[ts])
        {
    ...
    void del_timer(tw_timer *timer)
    {
        ...
        //slots[ts]是目标定时器所在槽的头节点，如果目标定时就是头节点，则
        //需要重置第ts个槽的头节点
        if(timer == slots[ts])
        {
    ...
    void tick()
    {
        ...
        cur_slot = (++cur_slot) % N; //更新时间轮的当前值，以反映时间轮的转动
    }
private:
    //时间轮上槽的数目
