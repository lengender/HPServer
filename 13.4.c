/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2016年10月10日 星期一 09时42分35秒
 ************************************************************************/
...
//子进程运行的函数，参数idx指出该子进程处理的客户连接编号，users是保存所有客户连接数据的
//数组，数组share_mem指出共享内存的起始地址
    ...
    while(!stop_child)
    {
        ...
        for(int i = 0; i < number; ++i)
        {
            ...
            if(sockfd == connfd && (events[i].events & EPOLLIN))
            {
                ...
                //将客户数据读取到对应的读缓冲区中，该缓冲区是共享内存的一段，它开始于
            ...
            else if(events[i].events & EPOLLIN)
            {
                ...
                else
                {
                    ...
                    for(int j = 0; j < user_count; ++j)
                    {
                        ...
                        {
                            printf("send date to child accross pipe\n");
